...
def run_experiment():
    ...
    base_train_log_dir = os.path.join(script_dir, "../train_log/LoRA_exp")
    base_result_dir = os.path.join(script_dir, "../result/LoRA_exp")
    ...
    print("Starting batch experiment for LoRA with different learning rates, ranks, and epochs...")
    print(f"Logging mode: {'File' if LOG_TO_FILE else 'Console'}")
    print(f"Learning rates to be tested: {learning_rates}")
    print(f"LoRA ranks to be tested: {lora_ranks}")
    ...
    for lr in learning_rates:
        for rank in lora_ranks:
            for num_epochs in num_epochs_list:
                ...
                exp_name = f"lr_{lr_str}_rank_{rank}_epochs_{num_epochs}"
                ...
                # --- Main Logic ---
                print(f"\n--- Starting Experiment for LR={lr}, Rank={rank}, Epochs={num_epochs} at {datetime.now()} ---")
                ...
                try:
                    ...
                    print(f"\n[ERROR] A step failed with exit code {e.returncode} for LR={lr}, Rank={rank}, Epochs={num_epochs}.")
                    ...
                    if log_file:
                        log_file.close()
                    print(f"--- Finished Experiment for LR={lr}, Rank={rank}, Epochs={num_epochs} at {datetime.now()} ---")
                    print("-" * 50)
